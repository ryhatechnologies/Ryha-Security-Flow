...
export class ComplianceValidator {
  ...
  public async validatePreScan(
    ...
    await this.auditLogger.log({
      eventType: EventType.AUTHORIZATION_VERIFIED,
      userId,
      jobId,
      details: {
      ...
      severity: result.valid ? Severity.INFO : Severity.CRITICAL,
      category: result.valid ? LogCategory.OPERATIONS : LogCategory.SECURITY
    });
   ...
   */
  private validateAuthorizationDocument(auth: AuthorizationDocument): Partial<ValidationResult> {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   */
  private checkExpiry(auth: AuthorizationDocument): Partial<ValidationResult> {
    const errors: string[] = [];
    const now = new Date();
   ...
   */
  private verifyDocumentIntegrity(auth: AuthorizationDocument): { valid: boolean } {
  ...
  public async validatePostScan(
    ...
    await this.auditLogger.log({
      ...
      severity: result.valid ? Severity.INFO : Severity.WARNING,
   ...
   */
  public verifyReportIntegrity(report: ComplianceReport): boolean {
    const { integrity, ...reportWithoutHash } = report;
   ...
   */
  public endSession(jobId: string): void {
  ...
  private isDomainAuthorized(domain: string, authorizedDomains: string[]): boolean {
  ...
  private isTargetExcluded(target: string, scope: ScopeDefinition): boolean {
...
// Singleton instance
let validatorInstance: ComplianceValidator | null = null;
...
export function setComplianceValidator(validator: ComplianceValidator): void {
