...
export const AgentPromptTemplates = {
  ...
  vulnerabilityExtraction: (toolName: string, scanType: string, target: string, phase: string) => `Analyze the following ${toolName} output from a ${scanType} scan on target ${target} during ${phase} phase.
...
Extract all security findings and return ONLY valid JSON in this exact format:
{
  ...
  toolSelection: (installedTools: string[], target: string, objective: string) => `You are a penetration testing expert. Select the best tools for this task.
...
[
  ...
  // Strategy planning
  strategyPlanning: (target: string, scanType: string, scope: string[]) => `You are an elite penetration tester planning a comprehensive security assessment.
...
Create a multi-phase attack strategy with tools, timing, and dependencies:
{
  ...
  customToolCreation: (language: string, purpose: string, target: string) => `You are an expert security tool developer.
