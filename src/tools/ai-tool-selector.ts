...
import { ToolManager, ToolInfo, ToolCategory } from './tool-manager';
import { GitHubInstaller } from './github-installer';
import { executeCommand, sanitizeTarget } from '../scanners/tool-wrappers';
 ...
 */
export type ToolTemplate = 'port-scanner' | 'web-fuzzer' | 'credential-tester' | 'api-enumerator' |
  'subdomain-finder' | 'vulnerability-checker' | 'network-sniffer' | 'log-analyzer' | 'hash-cracker' | 'custom';

/**
 * AIToolSelector - The autonomous brain for tool selection
 */
export class AIToolSelector extends EventEmitter {
  private toolManager: ToolManager;
  private discoveredTools: ToolInfo[] = [];
  private systemTools: Map<string, string> = new Map();
   ...
   */
  async discoverSystemTools(): Promise<Map<string, string>> {
    ...
    const checkPromises = securityToolNames.map(async (tool) => {
      try {
        const result = await executeCommand('which', [tool], 3000);
    ...
    for (const dir of toolDirs) {
      try {
        const result = await executeCommand('ls', [dir], 5000);
...
Return ONLY valid JSON:
{
  ...
  "fallbackStrategies": ["alternative approaches if primary fails"]
    ...
    try {
      const response = await copilotAuth.sendChatMessage(prompt, 'claude-opus-4-6');
...
Select 1-5 tools and provide exact commands. Return ONLY valid JSON:
{
  "tools": [
    ...
    try {
      const response = await copilotAuth.sendChatMessage(prompt, 'claude-3-5-sonnet-20241022');
...
Return ONLY valid JSON:
{
  ...
  async createCustomTool(
    purpose: string,
    target: string,
    language: 'bash' | 'python' | 'ruby' | 'perl' | 'go' = 'python',
    template?: ToolTemplate
  ): Promise<AIGeneratedTool> {
    const langGuidelines: Record<string, string> = {
...
Return ONLY valid JSON:
{
    ...
    try {
      ...
      if (language === 'go') {
        try {
          ...
          await executeCommand('go', ['build', '-o', binPath, savedPath], 60000);
          savedPath = binPath;
  ...
  async createToolFromTemplate(
    ...
    return this.createCustomTool(
      `${template} tool targeting ${target}`,
      target,
      language,
      template
    );
   ...
   */
  async ensureToolAvailable(toolName: string): Promise<{ available: boolean; method?: string }> {
    // Check if already installed
    try {
      const result = await executeCommand('which', [toolName], 3000);
      if (result.exitCode === 0 && result.stdout.trim()) {
    ...
    // Try apt-get install
    try {
      const aptResult = await executeCommand('sudo', ['apt-get', 'install', '-y', toolName], 120000);
    ...
    try {
      const pipResult = await executeCommand('pip3', ['install', '--break-system-packages', toolName], 60000);
    ...
    try {
      const goResult = await executeCommand('go', ['install', `github.com/projectdiscovery/${toolName}/cmd/${toolName}@latest`], 120000);
   ...
   */
  async analyzeAndDecideNextSteps(
    toolName: string,
    output: string,
    target: string,
    currentPhase: string,
...
Extract ALL security findings. Return ONLY valid JSON:
{
  ...
  private buildDefaultStrategy(
    ...
    // Phase 1: Recon
    const reconTools: ToolRecommendation[] = [];
    if (hasNmap) {
      reconTools.push({
    ...
    // Phase 2: Scanning
    const scanTools: ToolRecommendation[] = [];
    if (hasNuclei) {
      scanTools.push({
    ...
    const deepTools: ToolRecommendation[] = [];
