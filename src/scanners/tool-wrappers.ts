 ...
 */
export async function checkToolInstalled(toolName: string): Promise<boolean> {
  try {
 ...
 * @param toolName - Name of the tool
 */
export async function getToolVersion(toolName: string): Promise<string> {
  try {
 ...
 */
export function parseNmapXML(xml: string): Vulnerability[] {
  ...
  try {
    ...
    for (const hostMatch of hostMatches) {
      ...
      for (const portMatch of portMatches) {
        ...
        let severity: Vulnerability['severity'] = 'info';
        ...
        if (portId === 21) severity = 'medium'; // FTP
        if (portId === 23) severity = 'high';   // Telnet
        if (portId === 445) severity = 'medium'; // SMB
 ...
 * @param target - Target URL
 */
export function parseSqlmapOutput(output: string, target: string): Vulnerability[] {
  const vulnerabilities: Vulnerability[] = [];
 ...
 * @param target - Target URL
 */
export function parseGobusterOutput(output: string, target: string): Vulnerability[] {
  const vulnerabilities: Vulnerability[] = [];
 ...
 */
export function parseSSLScanOutput(output: string, target: string): Vulnerability[] {
  const vulnerabilities: Vulnerability[] = [];
 ...
 * @param target - Target URL
 */
export function parseWhatWebOutput(output: string, target: string): Vulnerability[] {
  const vulnerabilities: Vulnerability[] = [];
