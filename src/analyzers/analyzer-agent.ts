...
export class AnalyzerAgent {
  private static readonly OPUS_MODEL = 'claude-opus-4-6';
  private static readonly SONNET_MODEL = 'claude-3-5-sonnet-20241022';

  /**
   * Analyze scan results using AI
   */
  async analyzeScanResults(results: any[], target: string): Promise<AnalysisResult> {
    const prompt = this.buildAnalysisPrompt(results, target);

    try {
      const response = await copilotAuth.sendChatMessage(prompt, AnalyzerAgent.SONNET_MODEL);
   ...
   */
  async classifyVulnerability(vuln: any): Promise<VulnerabilityAnalysis> {
...
Respond ONLY with valid JSON, no additional text.`;
   ...
   */
  async detectZeroDays(findings: any[], target: string): Promise<ZeroDayAnalysis> {
...
Respond ONLY with valid JSON, no additional text.`;
    ...
    try {
      const response = await copilotAuth.sendChatMessage(prompt, AnalyzerAgent.OPUS_MODEL);
   ...
   */
  async correlateFindings(vulns: any[]): Promise<AttackChain[]> {
...
Respond ONLY with valid JSON containing the attackChains array, no additional text.`;
    ...
    try {
      ...
      return (parsed.attackChains || []).map((chain: any, index: number) => ({
        id: `chain-${index + 1}`,
        ...chain
      }));
    } catch (error) {
   ...
   */
  async generateExploitPOC(vuln: any): Promise<ExploitPOC> {
...
Respond ONLY with valid JSON, no additional text.`;
   ...
   */
  async assessRisk(vulns: any[]): Promise<RiskAssessment> {
...
Respond ONLY with valid JSON, no additional text.`;
   ...
   */
  private buildAnalysisPrompt(results: any[], target: string): string {
...
Provide your analysis in a structured format suitable for a professional penetration testing report.`;
   ...
   */
  private parseAnalysisResponse(response: string, originalResults: any[]): AnalysisResult {
    try {
   ...
   */
  private generateId(vuln: any): string {
   ...
   */
  private inferSeverity(vuln: any): VulnerabilitySeverity {
    if (vuln.severity) {
      return vuln.severity.toUpperCase() as VulnerabilitySeverity;
    }
