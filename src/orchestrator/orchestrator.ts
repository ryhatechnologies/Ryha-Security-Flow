...
import { AgentPool, Agent, AgentType } from './agent-pool';
import { AIToolSelector, AttackStrategy, ToolRecommendation } from '../tools/ai-tool-selector';
import { ScannerAgent } from '../scanners/scanner-agent';
...
export class PentestOrchestrator extends EventEmitter {
  private jobs: Map<string, PentestJob> = new Map();
  private agentPool: AgentPool;
  private copilotAuth: CopilotAuth;
  private authValidator: AuthValidator;
  private aiSelector: AIToolSelector;
  ...
  constructor() {
    ...
    this.aiSelector = new AIToolSelector(this.toolManager);
    this.scanner = new ScannerAgent({ verbose: true });

    // Listen to agent pool events
    this.agentPool.on('agent:output', (agentId: string, output: string) => {
    ...
    this.scanner.on('finding', (vuln: any) => {
      this.emit('scanner:finding', vuln);
    });

    this.scanner.on('output', (msg: string) => {
   ...
   */
  public async startPentest(
    target: string,
    scanType: 'full' | 'quick' | 'compliance' | 'web' | 'network',
    authDocId: string
  ): Promise<string> {
    // Validate authorization document and target scope
    const authValidation = await this.authValidator.validateBeforeScan(authDocId, target, scanType as any);
  ...
  private async executeJob(jobId: string): Promise<void> {
    ...
    try {
      ...
      for (const phase of job.phases) {
        ...
        if (job.status as string === 'stopped') {
          return;
        }
      }
   ...
   */
  private async executePhase(jobId: string, phase: PentestPhase): Promise<void> {
    const job = this.jobs.get(jobId);
    if (!job) throw new Error(`Job ${jobId} not found`);
    ...
    try {
      ...
      const agentPromises = phase.agents.map((agentTask) =>
        this.executeAgentTask(jobId, phase, agentTask)
      );
  ...
  private async executeAgentTask(
    ...
    try {
      ...
      const analysis = await this.analyzeTool(
        agentTask.name,
        output,
        {
          target: job.target,
   ...
   */
  private async waitForAgent(agent: Agent): Promise<void> {
  ...
  private async analyzeTool(
    tool: string,
    output: string,
    context: CopilotAnalysisRequest['context']
  ): Promise<any> {
    try {
      // Get Copilot token
      ...
      return { error: (error as Error).message, vulnerabilities: [] };
    }
  }
...
${output.substring(0, 5000)}`;
   ...
   */
  public getJobStatus(jobId: string): PentestJob | null {
   ...
   */
  private initializePhases(scanType: string): PentestPhase[] {
   ...
   */
  public async executeAIStrategy(
    jobId: string,
    target: string,
    scanType: string,
    scope: string[]
    ...
    try {
      ...
      for (const phase of strategy.phases) {
        ...
        for (const group of toolGroups) {
          const toolPromises = group.map(async (tool) => {
            try {
              ...
              const result = await this.scanner.runDynamicScan(
                tool.toolName,
                tool.args,
                target,
                { timeout: tool.timeout, requiresRoot: tool.requiresRoot }
              );
              ...
              this.emit('ai:tool-error', jobId, tool.toolName, (err as Error).message);
              return null;
            }
          });
        ...
        if (job.vulnerabilities.length > 0) {
          ...
          const nextTools = await this.aiSelector.analyzeAndDecideNextSteps(
            phase.name,
            JSON.stringify(job.vulnerabilities.slice(-10)),
            target,
            phase.name,
            completedToolNames
          );
      ...
      this.emit('ai:error', jobId, (error as Error).message);
   ...
   * Get all active jobs
   */
  public getActiveJobs(): PentestJob[] {
