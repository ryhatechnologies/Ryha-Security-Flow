...
exports.ToolManager = exports.ToolCategory = void 0;
...
class ToolManager {
    ...
    initializeToolDatabase() {
        ...
        const t = (name, displayName, desc, cat, caps, root = false, defArgs) => {
    ...
    async discoverUnknownTools() {
        ...
        for (const name of additionalTools) {
            if (this.installedTools.has(name) || this.toolDatabase.has(name))
                continue;
            const installed = await this.checkToolInstalled(name);
            if (installed) {
                const version = await this.getToolVersion(name);
    ...
    guessCategory(name) {
        ...
        if (/nmap|masscan|recon|amass|subfinder|harvest|shodan|whois|dig|dns|fierce/.test(lower))
            return ToolCategory.RECON;
        if (/nikto|nuclei|wpscan|joom|zap|burp|scan/.test(lower))
            return ToolCategory.SCANNER;
        if (/sql|xss|commix|dalfox|ffuf|wfuzz|gobust|dirb|ferox|fuzz|web|http|wafw|ssl|katana/.test(lower))
            return ToolCategory.WEB;
        if (/metasploit|msf|exploit|searchsploit|beef|router/.test(lower))
            return ToolCategory.EXPLOIT;
        if (/hydra|john|hashcat|medusa|ncrack|crack|cewl|crunch|hash|patator/.test(lower))
            return ToolCategory.PASSWORD;
        if (/air|wifi|reaver|bully|kismet|fluxion/.test(lower))
            return ToolCategory.WIRELESS;
        if (/wireshark|tshark|tcpdump|ettercap|bettercap|responder|sniff|mitm|hping|arp/.test(lower))
            return ToolCategory.SNIFFING;
        if (/autopsy|binwalk|volatility|foremost|strings|exif|steg/.test(lower))
            return ToolCategory.FORENSICS;
        if (/radare|ghidra|gdb|objdump|rizin|cutter/.test(lower))
            return ToolCategory.REVERSE_ENGINEERING;
        if (/enum4linux|smbclient|crackmapexec|evil-winrm|bloodhound|impacket|empire|sliver/.test(lower))
            return ToolCategory.POST_EXPLOITATION;
        if (/trivy|prowler|pacu|cloud|scout/.test(lower))
     ...
     */
    async createCustomTool(name, script, description, category = ToolCategory.CUSTOM) {
     ...
     */
    getToolCommand(toolName, options) {
    ...
    searchByCapability(capability) {
        return Array.from(this.installedTools.values()).filter(tool => tool.capabilities.some(cap => cap.includes(capability.toLowerCase())));
    }
    // Private helper methods
    async checkToolInstalled(name) {
        try {
        ...
        try {
            ...
            for (const file of files) {
                if (file.endsWith('.meta.json')) {
                    ...
                    const customTool = JSON.parse(content);
                    const toolInfo = {
