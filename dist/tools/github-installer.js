...
exports.GitHubInstaller = exports.KNOWN_GITHUB_TOOLS = void 0;
 ...
 */
class GitHubInstaller extends events_1.EventEmitter {
    constructor(installBase = '/opt/ryha-tools') {
        super();
     ...
     */
    async installFromGitHub(request) {
     ...
     */
    async installKnownTool(repoSlug) {
        const known = exports.KNOWN_GITHUB_TOOLS[repoSlug];
        if (!known) {
...
Prioritize actively maintained, well-known security tools.`;
            const response = await copilot_auth_1.copilotAuth.sendChatMessage(prompt, 'gpt-4o');
     ...
     */
    listKnownTools() {
        return Object.entries(exports.KNOWN_GITHUB_TOOLS).map(([repo, info]) => ({
            repo,
    ...
    async cloneOrUpdate(repoUrl, repoPath, branch) {
        try {
    ...
    async detectBuildSystem(repoPath) {
        ...
        if (fileSet.has('cargo.toml'))
            return 'rust';
        if (fileSet.has('setup.py') || fileSet.has('pyproject.toml') || fileSet.has('requirements.txt'))
            return 'python';
        if (fileSet.has('package.json'))
            return 'node';
        if (fileSet.has('gemfile') || fileSet.has('gemspec'))
            return 'ruby';
        if (fileSet.has('cmakelists.txt'))
            return 'cmake';
        if (fileSet.has('makefile') || fileSet.has('configure') || fileSet.has('configure.ac'))
            return 'make';
        // Check for scripts
    ...
    async buildAndInstall(repoPath, buildSystem, toolName, installPath, buildArgs) {
        switch (buildSystem) {
            case 'go':
    ...
    async buildGo(repoPath, toolName, installPath, buildArgs) {
    ...
    async buildPython(repoPath, toolName, installPath) {
    ...
    async buildRust(repoPath, toolName, installPath) {
    ...
    async buildNode(repoPath, toolName, installPath) {
        ...
        const wrapper = `#!/bin/bash\nnode ${path.join(repoPath, entryPoint)} "$@"\n`;
        await fs.writeFile(installPath, wrapper, { mode: 0o755 });
    }
    async buildRuby(repoPath, toolName, installPath) {
        const files = await fs.readdir(repoPath);
    ...
    async buildMake(repoPath, toolName, installPath) {
        const files = await fs.readdir(repoPath);
    ...
    async buildCMake(repoPath, toolName, installPath) {
    ...
    async installScript(repoPath, toolName, installPath) {
        // Find main script
        const files = await fs.readdir(repoPath);
        const script = files.find(f => f === `${toolName}.py` || f === `${toolName}.sh` || f === `${toolName}.rb` ||
            f === 'main.py' || f === 'main.sh' || f === toolName) || files.find(f => f.endsWith('.py') || f.endsWith('.sh') || f.endsWith('.rb'));
        if (!script) {
    ...
    async findPythonEntry(repoPath, toolName) {
        const candidates = [
    ...
    async getVersion(installPath, toolName) {
        try {
    ...
    async createSymlink(installPath, toolName) {
